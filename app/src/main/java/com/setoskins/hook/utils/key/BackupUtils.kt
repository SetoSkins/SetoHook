package com.setoskins.hook.utils.key
...
object BackupUtils {
    ...
    fun backup(activity: Activity, sp: SharedPreferences) {
        ...
        saveFile(
            activity,
            "SetoSkins-${SimpleDateFormat("yyMMdd_HHmmss").format(System.currentTimeMillis())}.json"
        )
    }
    ...
    fun handleReadDocument(activity: Activity, data: Uri?) {
        ...
        try {
            ...
            edit.apply()
            makeText(activity, "恢复成功", LENGTH_SHORT).show()
        } catch (e: Throwable) {
    ...
    fun handleCreateDocument(activity: Activity, data: Uri?) {
        ...
        try {
            ...
            makeText(activity, "备份成功", LENGTH_LONG).show()
        } catch (e: Throwable) {
